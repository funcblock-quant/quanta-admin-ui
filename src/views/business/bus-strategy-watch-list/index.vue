    ...
    <template #wrapper>
      <el-card class="box-card">
        <el-form ref="queryForm" :model="queryParams" :inline="true" label-width="68px">
          <el-form-item label="观察组" prop="symbolGroupId">
            <el-select
          ...
          <el-row :gutter="10" class="mb8">
            <el-col :span="1.5">
              <el-button
                v-permisaction="['business:busStrategyWatchList:add']"
                type="primary"
                icon="el-icon-plus"
                size="mini"
                @click="handleAdd"
          ...
          <el-table v-loading="loading" :data="busStrategyWatchList">
            <el-table-column
              label="币对组"
              align="center"
              prop="groupName"
            ...
            <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
              <template slot-scope="scope">
                <el-button
                  slot="reference"
                  v-permisaction="['business:busStrategyWatchList:edit']"
                  size="mini"
                  type="text"
                  icon="el-icon-edit"
                ...
                >
                  <el-button
                    slot="reference"
                    v-permisaction="['business:busStrategyWatchList:remove']"
                    size="mini"
                    type="text"
                    icon="el-icon-delete"
                  >删除
          ...
          <el-dialog :title="title" :visible.sync="open" width="500px">
            <el-form ref="form" :model="form" :rules="rules" label-width="100px">
              ...
              <el-form-item label="观察币种名称" prop="symbol">
                <el-input
                  v-model="form.symbol"
                  placeholder="观察币种名称"
                />
          ...
          <el-dialog :title="'批量添加观察币种'" :visible.sync="batchOpen" width="600px">
            <el-form ref="batchForm" :model="batchForm" label-width="100px">
              ...
              <el-form-item label="观察币种名称" prop="symbols">
                <el-input
                  v-model="batchForm.symbols"
                  type="textarea"
                  placeholder="请输入观察币种，每个币种用`;`分隔，例如：BTC/USDT;ETH/USDT"
                  rows="8"
                />
              </el-form-item>
            </el-form>
...
<script>
import { addBusStrategyWatchList, delBusStrategyWatchList, getBusStrategyWatchList, listBusStrategyWatchList, updateBusStrategyWatchList } from '@/api/business/bus-strategy-watch-list'
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      listBusStrategyWatchList(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
        this.busStrategyWatchList = response.data.list
    ...
    handleUpdate(row) {
      ...
      const id =
        row.id || this.ids
      getBusStrategyWatchList(id).then(response => {
        ...
        if (valid) {
          ...
          this.$confirm(confirmMessage, '提示', {
            ...
            if (this.form.id !== undefined) {
              updateBusStrategyWatchList(this.form).then(response => {
              ...
              addBusStrategyWatchList(this.form).then(response => {
                if (response.code === 200) {
                  this.msgSuccess(response.msg)
                  this.open = false
                  this.getList()
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除编号为"' + Ids + '"的数据项?', '警告', {
        ...
        return delBusStrategyWatchList({ 'ids': Ids })
      }).then((response) => {
        if (response.code === 200) {
          this.msgSuccess(response.msg)
          this.open = false
    ...
    submitBatchForm() {
      ...
      // 批量请求
      Promise.all(requestData.map(data => addBusStrategyWatchList(data)))
        .then(responses => {
